...
dat1 = read.csv('rna_data2.csv', head = T, stringsAsFactors = F)
source('hdknockoff_all_Factor.R')
library(glmnet); library(RSpectra); library(randomForest)
model.size = read.csv("model_size_multiple_d_rna2.csv", head = T, stringsAsFactors = F)
...
save.image(paste0('results_rna2_d', d, '.RData'))
