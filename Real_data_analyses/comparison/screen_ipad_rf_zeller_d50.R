...
dat1 = read.csv('Zeller_raw.csv', head = T, stringsAsFactors = F)
source('hdknockoff_all_Factor.R')
library(glmnet); library(RSpectra); library(randomForest)
model.size = read.csv("model_size_multiple_d_zeller.csv", head = T, stringsAsFactors = F)
...
for (ii in 1:nrep) {
  ...
  model.rf0 = randomForest(as.factor(label) ~ ., data = dat1[ind.train, c(ind.y, indvar1)], importance = T)
  ...
  fm = paste0('as.factor(label) ~ ', paste(vars, collapse = ' + '))
  ...
  pe.rf1[ii] = mean(pred.model.rf != dat1[ind.test,'label'])
  
  ## 2. Random Forest with top model size variables from screening ##
  indvar2 = top500[1:size1[ii],ii]
  model.rf2 = randomForest(as.factor(label) ~ ., data = dat1[ind.train, c(ind.y, indvar2)], importance = T)
  pred.model.rf2 = predict(model.rf2, newdata = dat1[ind.test,]) #prediction
  pe.rf2[ii] = mean(pred.model.rf2 != dat1[ind.test,'label'])
  
  ## 3. Random Forest ##
  pred.model.rf3 = predict(model.rf0, newdata = dat1[ind.test,]) #prediction
  pe.rf3[ii] = mean(pred.model.rf3 != dat1[ind.test,'label'])
  ...
  if (length(Supp_plus) > 0) {
    obj.net = cv.glmnet(as.matrix(dat1[ind.train,names(Supp_plus)]), dat1[ind.train, 'label'], family='binomial', alpha = 1) 
    ...
    pe.ipad[ii] = mean(pred.model.ipad != dat1[ind.test,'label'])
  } else {
    pred.model.ipad = rep(as.numeric(mean(dat1$label[ind.train]) > 0.5), length(ind.test))
    pe.ipad[ii] = mean(pred.model.ipad != dat1[ind.test,'label'])
  }
  #########################################################
}
...
save.image(paste0('results_zeller_d', d, '.RData'))
